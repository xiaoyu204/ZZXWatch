...
{
    HAL_I2C_Master_Transmit(&OLED_I2C, OLED_ADDRESS, data, len, HAL_MAX_DELAY);
}
...
{
    memset(OLED_GRAM, mode, sizeof(OLED_GRAM));
}
...
{
    OLED_SetBlock(x, y, font->chars + (ch - ' ') * (((font->h + 7) / 8) * font->w), font->w,
                  font->h, color);
}
...
{
    char Data[sizeof(Fnum)];                   // 数据位数由sizeof(Fnum) 来判断，灵活创建数组大小
    sprintf(Data, "%.1f", Fnum);               // 保留小数点后1位小数，打印到Data数组中
    OLED_PrintString(x, y, Data, font, color); // 调用oled字符显示函数，在OLED屏上逐个显示
}
...
void OLED_PrintNum(uint8_t x, uint8_t y, uint32_t num, const Font *font, OLED_ColorMode color)
{
    ...
    for (t = 0; t < len; t++) {
        ...
        str[0] = temp + '0';
        // sprintf(str, "%d", temp - '0' + 48);
        OLED_PrintString(x + font->w * t, y, str, font, color);
    }
}
